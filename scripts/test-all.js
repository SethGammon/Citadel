...
const OPENCODE_SKILL_COMMAND_TEST = path.join(PLUGIN_ROOT, 'scripts', 'test-opencode-skill-command.js');
...
const UNLOCK_TESTS = Object.freeze([
  ...
  ['MCP handshake revision matrix', 'test-mcp-handshake-matrix.js'],
  ['MCP protocol adapter', 'test-mcp-protocol-adapter.js'],
  ['MCP server protocol conformance', 'test-mcp-server-protocols.js'],
  ['MCP external conformance provenance', 'test-mcp-conformance-pins.js'],
  ['MCP package closure', 'test-mcp-package-closure.js'],
...
const opencodeSkillCommandPassed = run('opencode Skill Command Tests', OPENCODE_SKILL_COMMAND_TEST);
...
const upgradeSafetyPassed = run('Version Upgrade Safety Tests', path.join(PLUGIN_ROOT, 'scripts', 'test-version-upgrades.js'));
const sessionSafetyPassed = run('Session Safety Tests', path.join(PLUGIN_ROOT, 'scripts', 'test-session-safety.js'));
const unattendedSafetyPassed = run('Unattended Safety Tests', path.join(PLUGIN_ROOT, 'scripts', 'test-unattended-safety.js'));
...
console.log(`  opencode skill cmd: ${opencodeSkillCommandPassed ? 'PASS' : 'FAIL'}`);
...
console.log(`  Session safety: ${sessionSafetyPassed ? 'PASS' : 'FAIL'}`);
console.log(`  Unattended safety: ${unattendedSafetyPassed ? 'PASS' : 'FAIL'}`);
...
if (!opencodeSkillCommandPassed) console.log('opencode skill command tests failed. Fix slash-command interception in runtimes/opencode/plugin/skill-command.js before proceeding.');
