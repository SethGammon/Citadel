...
| **Interoperable** | **Locally proven; registry gate failed** | The [interoperability contract](INTEROPERABILITY.md) passes an unmodified external skill through Claude and Codex projections, with immutable provenance and drift-checked metadata. ClaudePluginHub indexes version `1.1.0`, while HOL accepts the repository with trust score 92. Live inspection also shows the Claude listing unclaimed and HOL publisher verification `No` with stale version `1.0.0`. | Claim the ClaudePluginHub publisher profile with explicit account authorization; publish and refresh HOL only after the release gates pass. |
| **Releasable** | **CI and local foundations proven; blocked** | Tests and HOL checks are green. The clean worktree produced a reproducible 617-file `1.1.0` archive and passed independent manifest/checksum verification; migration/update, backup, and rollback commands are documented in [release operations](RELEASES.md). | There is still no `v1.1.0` tag, published release artifact, verified publisher profile, or fresh-clone all-OS checksum proof. |
