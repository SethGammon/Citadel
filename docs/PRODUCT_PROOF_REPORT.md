...
|---|---|---|---|
| **Reliable** | **Locally proven; hosted regression pending** | The final integrated `node scripts/test-all.js --strict` run passed every listed check in 214.6 seconds, including the repaired fail-closed aggregate exit. Release integrity, reproducibility, updater, and rollback contracts are described in [release operations](RELEASES.md). | Push the integrated commit and get the required hosted CI checks green. |
| **Installable** | **Blocked** | The workflow collects Claude and Codex runs on Linux, macOS, and Windows and enforces the [complete 30-run matrix](../.github/workflows/tests.yml#L29-L80). The first actual grid was complete, but only 15/30 passed: every Claude run passed and every Codex install failed because the generated artifact was skipped in a fresh checkout. The repair now passes focused local tests. | Rerun PR #181 and publish a green 30-run artifact; the gate is greater than 95%, so 15/30 does not qualify. |
...
| **Resumable** | **Locally proven; CI proof pending** | The local golden-path fixture verifies handoff, fresh-process continuation, and rollback; see [golden-path evidence](GOLDEN_PATH.md). | The repaired cross-OS rerun must show greater than 95% fresh-session resume success. |
| **Understandable** | **Blocked** | Nine versioned, read-only dashboard contracts pass. Focused 1,000-file runs measured 251.9-588.4 ms cold start and 110.8-228.5 ms invalidated updates; [dashboard evidence](DASHBOARD_SPEC.md#verification-evidence) records 53.3-53.5 MB absolute RSS against the strict `<50 MB` target. Browserless keyboard, responsive, and reduced-motion contracts pass. | Meet or explicitly revise the RSS gate with platform evidence; capture pixel screenshots once browser security allows it; prove at least 8/10 strangers identify state and next action in under 60 seconds. |
...
- PR #181's first hosted golden-path result is negative evidence, not a pass. A local
  repair does not become CI proof until the rerun artifact passes.
