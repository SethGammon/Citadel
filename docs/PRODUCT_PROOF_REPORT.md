...
|---|---|---|---|
| **Reliable** | **CI-proven** | At commit `77ebbb2`, Tests run 37 passes the strict Node 18/20 matrix on Linux, macOS, and Windows. The local aggregate also passed every listed check in 206.0 seconds, including the repaired fail-closed exit. | Preserve these required checks on the eventual milestone commit. |
